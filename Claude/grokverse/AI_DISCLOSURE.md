...
| `RESULTS.md`, `README.md` (current text) | AI-drafted, and **still describing the pre-study reproduction phase**; several claims are classified as over-interpreted by `docs/CURRENT_EVIDENCE_AUDIT.md` and carry inline `[AUDIT]` marks. The analyses they were waiting on are now complete, so the rewrite is due |
