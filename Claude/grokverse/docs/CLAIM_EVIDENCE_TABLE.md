...
| **Quantitative evidence** | MLP `remove_structured`: observed test-accuracy drop **0.991**, size-matched random control **0.878**. Transformer `remove_structured_neurons`: **0.942** vs control **0.910** (control values as re-run after the 2026-09-06 determinism fix, D8 — `docs/LABBOOK.md` 1175; aligned with `RESULTS.md` §6 on 2026-09-20). `keep_structured` retains 1.000 / 0.9997 — but so does a random set of the same size (control drop 0.000). Structured set: 442/512 (MLP), 501/512 (transformer); for transformer seed 5 it is **all 512**, so no control set exists and the value is `null`. |
