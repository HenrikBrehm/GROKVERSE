...
| D8 | **A determinism defect in the frozen control code, found 2026-09-06, moves two published control numbers** | `causal_ablation.resolve_structured_masks` built the set of available structured definitions as a Python **set**, and both ablation functions iterate that dict while drawing from **one shared `rng`**. Python randomises string hashing per process, so which 50 size-matched control draws went to which definition changed from run to run. **Observed ablation values were never affected** — they are identical to the last decimal — but the random-control statistics were not reproducible: re-running the module on identical inputs moved `remove_structured` control mean drop by up to **0.0034** (MLP) and `remove_structured_neurons` by **0.0015** (transformer), which exceeds the ±0.001 tolerance that `tests/check_results_numbers.py` asserts against `RESULTS.md` §6. The defect predates the analysis freeze (`ac5bec4`) and was invisible because the module was never re-run in a fresh process and diffed. **The AI treated the one-line ordering fix as a genuine bug fix under `PREREGISTRATION.md` §9** — a control that is not reproducible is not a control, and determinism is a stated non-negotiable of the project constitution — bumped `MODULE_VERSION` to 1.2, re-ran all 40 primary checkpoints, and updated the two affected numbers in `RESULTS.md` and in the verifier, each logged in `docs/LABBOOK.md`. **Nothing scientific changes:** both controls stay at ≈ 0.87 (MLP) and ≈ 0.91 (transformer), the reading "the size-matched control alone already does most of the damage" is unchanged, and G4 still reads **0/10** in both architectures. The human authors decide whether they accept the fix as a §9 bug fix rather than a control change, and whether the two updated numbers are re-checked independently before submission. | |
| D9 | **Externally executed follow-up blocks (S1 at 100,000 steps; two-hot ×10) and the reviewer's S2/S2b/S3 analyses — do they enter the study, and on what terms?** | Received 2026-09-08 (`docs/external/README.md`, `RESULTS.md` §16). The AI has (a) verified the bundle's integrity and provenance (30 manifests at `42dd79a` = `fda066e` + the checkpoint-grid extension; 829/829 checkpoint hashes; split hashes identical to ours per seed), (b) re-derived the gate, statistics, H3 and H4 reports with the frozen code (byte-identical), (c) reported everything as a separate, labelled section without changing any 25k number, verdict or code, and (d) **not** merged the reviewer's `CHECKPOINT_GRID` extension. The reviewer's own runbook §0 binds: external compute is disclosed (done, `AI_DISCLOSURE.md` §5/§9); interpretation stays with the students; **at least one seed pair is reproduced on the authors' hardware before any S1 number enters a submission** (not done; ≈ 3.6 h for an MLP seed and ≈ 16 h for a transformer seed at 100k on this machine, from a worktree of the local `arch-study-convergence` branch); whether externally executed compute is admissible under the competition rules is the authors' question (not checked). Open in addition: the S2/S2b/S3 scripts are not in the bundle (requested 2026-09-08), so those numbers are *reported*, not re-derived; whether the reviewer is named (the bundled runbook and `docs/RESEARCH_SPEC.md` do, the rest of the repository does not); whether the reviewer becomes a collaborator on the public repository (asked 2026-09-08). **AI proposal:** keep §16 as reported; do not merge the grid; cite S2/S2b/S3 only as reported until the scripts arrive; run the seed-0 pair reproduction once the compute is authorised; settle the competition-rules question before the write-up uses any 100k number. | `[HUMAN 2026-09-20, verbal, recorded by the AI]` The reviewer is named: **Dr. Jana Stucke** (README, RESULTS §6/§16, AI_DISCLOSURE §9). The other D9 questions — accepting the block on the runbook's terms, merging the grid, the collaborator request — remain open. |
