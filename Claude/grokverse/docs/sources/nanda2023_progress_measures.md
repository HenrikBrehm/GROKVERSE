...
**Independent re-verification (2026-09-03, second pass) [verifier 2026-09-03].** All sources were re-fetched from scratch and every load-bearing statement re-checked by exact string match against the fresh dumps:
* `https://arxiv.org/html/2301.05217` re-downloaded with `curl` (577,635 bytes; identical byte count to `.../2301.05217v3`, i.e. the served HTML *is* v3) and converted to text locally with LaTeXML `alttext` kept as `$...$`. Every quotation in §§1, 3–8 was re-checked; **no wrong quote, wrong number or wrong section reference was found in the paper quotations.** Verified individually: §3 setup paragraph, §3.1 all bullets + eq. (1), §4.1 (embedding/logit periodicity, Fig. 3 and Fig. 4 captions), §4.2 + eqs. (2)/(3) + Table 1 (all ten FVE values), §4.3, §4.4 (all five ablation numbers), §5.1 restricted + excluded paragraphs in full, §5.2 all three phase paragraphs, Fig. 6/7/18 captions, App. A.1 (all five ablation numbers), App. C.2 + footnote 3, App. C.2.1 (3-or-4 key frequencies, "6 or 8 directions", Table 3, Table 4), App. C.2.2 (data fractions, primes), App. D.1 (weight-decay and dropout/ℓ1 results).
* GitHub `neelnanda-io/Grokking`: file list and byte sizes re-checked via the API (identical), README re-fetched verbatim (identical).
* The Colab notebook re-downloaded via the Drive export (41,202,464 bytes, **346 cells**, saved outputs present). Every quoted cell index was re-checked and is correct: 12, 16, 69, 71, 74 (+ output `3830 8939`), 75, 76, 85 (+ output `2.4122030061370614e-07`), 91, 121, 122, 172, 173, 175, 176, 179, 180 (+ cluster output), 191 (+ output), 193, 197 (+ output `5.547432139695308e-08`), 198, 199, 204, 212, 215, 216, 220, 226, 244, 245, 247, 248. **One location error found and fixed below (the "norm 1" markdown is cell 120, not 121).**
* TransformerLens `demos/Grokking_Demo.ipynb` re-downloaded (5,921,470 bytes, 140 cells); `get_restricted_loss` (cell 131) and `get_excluded_loss` (cell 137) re-read in full, together with cells 14, 23, 25/26, 28, 32, 64, 70, 72, 108.
* Blog re-fetched via the greaterwrong mirror; all twelve quoted passages matched by exact string search.
Additions and corrections made in this pass are tagged `[verifier 2026-09-03]`. The writer's caveats are unchanged.
...
```
Row layout: 0 = constant, then `(cos k, sin k)` at rows `2k−1, 2k` for k = 1..56 — the same layout as GROKVERSE `fourier.fourier_basis`. ~~Cell 121 markdown~~ **Cell 120 markdown [verifier 2026-09-03 — location corrected; cell 121 is the code cell, the prose is in cell 120]**: "Each term is scaled to have norm 1." (its next sentence says "Note that the constant term is scaled by 1/p, the rest by √2/p" — that is the notebook's own wording; the code divides by the actual norm, i.e. 1/√p and √(2/p)). Cell 122: "Crucially, this is an orthonormal basis, so we can transform any p dimensional vector to this basis - this is the 1D Fourier transform" **[verifier 2026-09-03: full sentence]**.

**[verifier 2026-09-03] Also in cell 120, load-bearing for GROKVERSE's basis choice:** "**Aside:** Note that cos(wx+α)=A cos(wx)+B sin(wx) for some constants A and B. As modular addition is periodic, this means that the model has no reason to privilege cos(wx),sin(wx) as a basis for the 2D space of 'waves of frequency w'. It's nicer to have a basis than a set of orthogonal 2D subspaces, so we use sin and cos for convenience, but there's nothing privileged about it. However, the 2D subspace spanned by them *is* privileged." — i.e. the author states explicitly that only the 2D per-frequency *subspace* is meaningful, not the individual cos/sin coordinates. This is the source-side justification for `mask_protocols` treating the sum directions as a **projection onto a subspace** rather than a coordinate mask.
